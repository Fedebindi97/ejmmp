...
if plannerMethods.externalOrderGeneration:
    import redis
...
class CentralPlanner(core.Agent):
    ...
    def diffusingProductionOrders(self):
        ...
        if plannerMethods.externalOrderGeneration:
            ...
            float_str = data.split("Floats: [")[1].replace(']','')
            ...
            received_orders = {} # storing the orders that the firms received
      
            for i,aFirm in enumerate(context.agents(agent_type=0)): 

                firm_float = float_list[i]
                order_quantity, order_duration = aFirm.maxOrderProduction*params["minOrderAsAShareOfMaxOrderProduction"] + \
                        firm_float * aFirm.maxOrderProduction*(1 - params["minOrderAsAShareOfMaxOrderProduction"]),\
                         rng.integers(aFirm.minOrderDuration, aFirm.maxOrderDuration+1) * plannerMethods.durationCoeff

                if plannerMethods.askingInvGoodsProduction == 'regular':
                    aFirm.receivingNewOrder(order_quantity, order_duration)
                ...
                received_orders[i] = (order_quantity, order_duration)
            ...
            if 'Step: '+str(t()+1) in data:
                result = f"Python {t()+1}: {received_orders}"
                print(f"Python computing step {t()+1}...")
                print(f"Python sending step {t()+1}")
...
# run the model
# 
# ### if multi-rank, from the terminal launch: mpirun -n x ipython model1.2ghost.ipynb  
# 
...
# #do not delete, useful to access the model and its elements; use it in substitution of the previous cell
# model = Model(params)
# model.start()

# #an example
# model.investmentGoodPrices

# #do not delete, useful to access the agents (you can use the regular cell to start)
# a=list(context.agents(agent_type=0))
